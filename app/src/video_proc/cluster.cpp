...
Bounds2i FindBounds(const std::vector<ClusterPixel>& pixels) {
	if (pixels.empty()) {
		return {};
	}
	...
	for (const auto& pixel : pixels) {
		ExpandToIncludePoint(bounds, pixel.loc);
	}

	return bounds;
}

ClusterMetrics ClusterMetrics::FromWeightedPixels(const std::vector<ClusterPixel>& accepted) {
	if (accepted.empty()) {
		return {};
	}
	...
	metrics.bounds = FindBounds(accepted);
...
ClusterMetrics ClusterMetrics::FromPixels(const std::vector<ClusterPixel>& accepted) {
	if (accepted.empty()) {
		return {};
	}
...
void ClusterFill(cv::Mat& img,
	...
	while (!todo_list.empty()) {
		...
		pt = todo_list.front();
		accepted.emplace_back(Vec2i{pt.x, pt.y}, img.at<uchar>(pt));
